...
static cl::list<std::string> SLHConfig("slh-config",
                                     cl::desc("String of 0/1 to set whether or not to harden.\n"
                                              "eg:\n"
                                              "   fname:1011"),
                                     cl::Hidden);
...
private:
  ...
  unsigned hardenCount = 0;
  std::deque<bool> bitDeque;
  std::deque<bool> stringToBitDeque(const std::string& bitString);
...
std::deque<bool> AArch64SpeculationHardening::stringToBitDeque(const std::string& bitString) {
    //std::deque<bool> bitDeque;
    for (char bit : bitString) {
        bitDeque.push_back(bit == '1');
    }
    return bitDeque;
}
...
bool AArch64SpeculationHardening::runOnMachineFunction(MachineFunction &MF) {
  ...
  llvm::StringRef slh_input;
  if (!SLHConfig.empty()) {
    for (llvm::StringRef slh_config_input : SLHConfig) {
      slh_input = "";
      std::pair<StringRef, StringRef> slh_config = slh_config_input.split(':');
      if (slh_config.first == MF.getFunction().getName()) {
        slh_input = slh_config.second;
        break;
      }
    }
  }
  hardenCount = 0;
  bitDeque.clear();
  bitDeque = stringToBitDeque(slh_input.str());
  ...
  llvm::outs() << "harden count: " << MF.getFunction().getName() << ":" << hardenCount << "\n";
